...
async def treeselect(request: Request, current_user: CurrentUser = Depends(get_current_user)) -> dict[str, Any]:
    """菜单树下拉。"""
    return await service.treeselect(request=request, current_user=current_user)


@router.get("/system/menu/roleMenuTreeselect/{roleId}", summary="角色菜单树。")
async def role_menu_treeselect(roleId: int, request: Request, current_user: CurrentUser = Depends(get_current_user)) -> dict[str, Any]:
    """角色菜单树。"""
    return await service.role_menu_treeselect(roleId=roleId, request=request, current_user=current_user)


@router.get("/system/menu/{menuId}", summary="查询菜单详情。")
async def get(menuId: int, request: Request, current_user: CurrentUser = Depends(get_current_user)) -> dict[str, Any]:
    """查询菜单详情。"""
    return await service.get(menuId=menuId, request=request, current_user=current_user)
...
async def delete(menuId: int, request: Request, current_user: CurrentUser = Depends(get_current_user)) -> dict[str, Any]:
    """删除菜单。"""
    return await service.delete(menuId=menuId, request=request, current_user=current_user)
