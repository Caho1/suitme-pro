...
async def update_orders(request: Request, current_user: CurrentUser = Depends(get_current_user)) -> dict[str, Any]:
    """批量调整标签顺序。"""
    return await service.update_orders(request=request, current_user=current_user)
