...
def ajax_result(code: int = 200, msg: str = "操作成功", **kwargs: Any) -> dict[str, Any]:
    """构建松散结构的 AjaxResult。"""
...
def success(data: Any = None, msg: str = "操作成功", **extra: Any) -> dict[str, Any]:
    """返回成功结果。"""
    payload = ajax_result(code=200, msg=msg, **extra)
    if data is not None:
        payload["data"] = data
    return payload
...
def table_data(rows: list[Any], total: int, msg: str = "查询成功") -> dict[str, Any]:
    ...
    return {"code": 200, "msg": msg, "total": total, "rows": rows}
...
def captcha_disabled() -> dict[str, Any]:
    """返回固定关闭验证码的兼容结构。"""
    return ajax_result(code=200, msg="操作成功", captchaEnabled=False, uuid="", img="")


def not_implemented(feature: str) -> dict[str, Any]:
    """返回统一的“骨架已创建”提示。"""
    return error(msg=f"接口骨架已创建，待迁移业务逻辑：{feature}", code=500)
